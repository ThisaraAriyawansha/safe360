...
export async function sendPush(sensorId) {
  initAdmin();

  const db = getDatabase();
  const tokensSnap = await db.ref("safe360/tokens").get();
  if (!tokensSnap.exists()) return { sent: 0, total: 0 };
  ...
  if (staleKeys.length > 0) {
    await Promise.all(staleKeys.map(k => db.ref(`safe360/tokens/${k}`).remove()));
  }

  console.log(`[push] ${label}: ${response.successCount}/${tokens.length} sent, ${staleKeys.length} stale removed`);
